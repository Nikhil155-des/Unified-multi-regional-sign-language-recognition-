...
#         """Gets the list of class names (word directories only)."""
#         classes = sorted([
...
import numpy as np
from .video_preprocessing import process_video, sample_frames
from utils.config import MAX_FRAMES, TOTAL_KEYPOINT_DIM, TRAIN_DIR, TEST_DIR
from .pose_estimation import PoseEstimator
import os
import sys
...
class SignLanguageDataset(Dataset):
    def __init__(self, root_dir, sequence_length=30, transform=None):
        ...
        print(f"[DEBUG] Listing dirs in: {self.root_dir}")
        print(os.listdir(self.root_dir))  # Raw view of what's inside

        self.classes = self._get_classes()  # Get classes (word directories)
        self.class_to_idx = {cls: idx for idx, cls in enumerate(self.classes)}

        print(f"[DEBUG] Final class list: {self.classes}")
        print(f"[DEBUG] class_to_idx: {self.class_to_idx}")

        self.video_data = self._load_video_data()  # Load video paths and labels

    def _get_classes(self):
        """Gets the list of class names (word directories only), normalized to lowercase."""
        classes = sorted([
            d.strip().lower() for d in os.listdir(self.root_dir)
            if os.path.isdir(os.path.join(self.root_dir, d))
        ])
    ...
    def _load_video_data(self):
        ...
        for word_dir in os.listdir(self.root_dir):
            word_path = os.path.join(self.root_dir, word_dir)
            if os.path.isdir(word_path):
                class_name = word_dir.strip().lower()  # Normalize for lookup
                label = self.class_to_idx.get(class_name)
                if label is None:
                    print(f"[WARNING] Skipping {word_path} - class '{class_name}' not in class_to_idx")
                    continue
                for video_file in os.listdir(word_path):
                    if video_file.endswith(".mp4"):
                        video_path = os.path.join(word_path, video_file)
                        video_data.append((video_path, label))
                        print(f"Found: {video_path}, Label: {label}")
    ...
    def __getitem__(self, idx):
        ...
        keypoint_sequences = np.array(keypoint_sequences, dtype=np.float32)
